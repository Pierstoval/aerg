...
import { HexagonalAngle } from './tiling/HexagonalCoordinateSystem';
...
export default class Player {
	...
	get topCoord(): number {
		return (
			this._height / 2 -
			this._tileset.tileHeight / 2 +
			yCoordInPixels(this._x, this._y, this._tileset.tileHeight, this._tileset.hcs.angle)
		);
	}
	...
	get leftCoord(): number {
		return (
			this._width / 2 -
			this._tileset.tileWidth / 2 +
			xCoordInPixels(this._x, this._y, this._tileset.tileWidth, this._tileset.hcs.angle)
		);
	}
	...
	public moveDownLeft(): void {
		if (this._tileset.hcs.angle === HexagonalAngle.pointy) {
